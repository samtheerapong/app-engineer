<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;

/* @var $this yii\web\View */
/* @var $model app\modules\engineer\models\MachineSearch */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="machine-search">

    <?php $form = ActiveForm::begin([
        'action' => ['index'],
        'method' => 'get',
    ]); ?>

    <?= $form->field($model, 'id') ?>

    <?= $form->field($model, 'machine_code') ?>

    <?= $form->field($model, 'machine_name') ?>

    <?= $form->field($model, 'machine_en_name') ?>

    <?= $form->field($model, 'machine_details') ?>

    <?php // echo $form->field($model, 'machine_type_id') ?>

    <?php // echo $form->field($model, 'repair_id') ?>

    <?php // echo $form->field($model, 'serial_number') ?>

    <?php // echo $form->field($model, 'station_id') ?>

    <?php // echo $form->field($model, 'po_number') ?>

    <?php // echo $form->field($model, 'cost') ?>

    <?php // echo $form->field($model, 'installation_date') ?>

    <?php // echo $form->field($model, 'docs') ?>

    <?php // echo $form->field($model, 'photos') ?>

    <?php // echo $form->field($model, 'machine_status_id') ?>

    <div class="form-group">
        <?= Html::submitButton(Yii::t('app', 'Search'), ['class' => 'btn btn-primary']) ?>
        <?= Html::resetButton(Yii::t('app', 'Reset'), ['class' => 'btn btn-outline-secondary']) ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>
